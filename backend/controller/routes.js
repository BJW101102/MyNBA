...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const insertNewUser =
      "Insert INTO UserInfo (username,password) VALUES (?, ?)"; // SQL Query
    db.run(insertNewUser, [username, hashedPassword], function (err) {
      //Running Query
      if (err) {
...
router.get("/userdata", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ username: user.username, userID: user.userID });
  } catch (error) {
...
router.put("/follow/:id", async (req, res) => {
    try {
        ...
        const insertUserTeam = "INSERT INTO UserTeams (userID, teamID) VALUES (?, ?)";
...
router.delete("/unfollow/:id", async (req, res) => {
    try {
        ...
        const user = req.session.user;
        const teamID = Number.parseInt(req.params.id);
        const deleteUserTeam = "DELETE FROM UserTeams WHERE userID = ? AND teamID = ?";
...
router.get("/followed-teams", async (req, res) => {
    try {
        ...
        // Creating user reference
        const user = req.session.user;
        const selectUserTeams = "SELECT teamID FROM UserTeams WHERE userID = ?";
        req.db.all(selectUserTeams, [user.userID], function (err, rows) {
            if (err) {
                return res.status(500).json({ message: "Error fetching user teams" });
            }
            return res.status(200).json(rows.map(x => x.TeamID));
        });
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
